...
print('dataset, samples, features , labels, unique rankings, auc_scores, runtime')
for csv_name in csv_choices:
    ...
    out_string += f'{np.round(np.mean(auc_scores),3)}\u00B1{np.round(np.std(auc_scores),3)}, {run_time}s'
    print(out_string)
