...
params = {                                                          # TOSET
    ...
    'kwik_num': 0 if len(sys.argv) < 4 else int(sys.argv[3])
}
...
def kemeny_rank_aggregation(rs):
...
def kwiksort2(row):     # use many kwiksorts with random pivots  and aggregate
    """Given binary preference between labels, 
    construct the relevant majority tournament and implement kwiksort
    """
    # produce adjeceny matrix of tournament (weights)
    ...
    def recurse(arr):
        if len(arr) <= 1:
            return arr
        rand_idx = np.random.randint(0, len(arr)-1)            # random pivot
        arr[-1], arr[rand_idx] = arr[rand_idx], arr[-1] # put at the end
        slow, fast = 0, 0 # slow and fast pointer and pivot = arr[-1] 
        while fast < len(arr) - 1:
            start_node, end_node = arr[fast], arr[-1]
            if adj_matrix[start_node][end_node] == 1: 
    ...
    for i in range(2*n_labels):
        ...
        aggregate_pf = recurse(begin_pf)
        aggregate_lf = list(np.argsort(aggregate_pf))
        rankings.append(aggregate_lf)
    return kemeny_rank_aggregation(rankings)
...
for idx_train,idx_test in rkf.split(range(len(df))):
    ...
    if params['kwik_num'] == 0:
        test_df['prediction'] = test_df.apply(lambda row: kwiksort(row), axis=1) 
    else: 
        test_df['prediction'] = test_df.apply(lambda row: kwiksort2(row), axis=1) 
